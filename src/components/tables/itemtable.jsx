...
const ItemTable = (props) => {
  ...
  const [selectedLarpTag, setSelectedLarpTag] = useState(
    props.Filters !== null &&
      props.Filters !== undefined &&
      props.Filters.SelectedLarpTag !== undefined
  ...
  const [tagSelectValues, setTagSelectValues] = useState(
    props.Filters !== null &&
      props.Filters !== undefined &&
      props.Filters.TagSelectValues !== undefined
      ? props.Filters.TagSelectValues
      : [],
  );
  const [selectedApprovalState] = useState(
    props.Filters !== null &&
      props.Filters !== undefined &&
      props.Filters.SelectedApproval !== undefined
      ? props.Filters.SelectedApproval
      : null,
  );
  ...
  const [larpAutoCompValue, setLarpAutoCompValue] = useState(
    props.Filters !== null &&
      props.Filters !== undefined &&
      props.Filters.LarpAutoCompValue !== undefined
  ...
  useEffect(() => {
    ...
    if (!props.isSearch && selectedLarpTag !== null && selectedLarpTag !== "") {
      const fullSelectLARPTag = props.larpTags.find(
        (tag) => tag.guid === selectedLarpTag,
      );
    ...
    if (props.Filters !== undefined && props.Filters !== null) {
      if (props.Filters.ItemsFilter != undefined) {
        filteredRows = filteredRows.filter((item) =>
          removeDiacritics(item.name.toLocaleLowerCase()).includes(
            removeDiacritics(props.Filters.ItemsFilter.toLocaleLowerCase()),
          ),
        );
      }
      if (props.Filters.SeriesFilter !== undefined) {
        filteredRows = filteredRows.filter(
          (item) =>
            (item.series === null && props.Filters.SeriesFilter === "") ||
            (item.series !== null &&
              removeDiacritics(item.series.toLocaleLowerCase()).includes(
                removeDiacritics(
      ...
      if (props.Filters.CreatorFilter !== undefined) {
        filteredRows = filteredRows.filter((item) =>
          removeDiacritics(item.createdby.toLocaleLowerCase()).includes(
            removeDiacritics(props.Filters.CreatorFilter.toLocaleLowerCase()),
          ),
        );
      }
      if (props.Filters.EditorFilter !== undefined) {
        filteredRows = filteredRows.filter((item) =>
          removeDiacritics(item.editbyUser.toLocaleLowerCase()).includes(
            removeDiacritics(props.Filters.EditorFilter.toLocaleLowerCase()),
          ),
        );
      }
    }
    if (props.showApprovableOnly) {
      filteredRows = filteredRows.filter(
        (item) =>
          item.editbyUserGuid !== props.userGuid &&
          item.firstapprovalbyuserGuid !== props.userGuid,
      );
    }
    if (props.readyApproved) {
      filteredRows = filteredRows.filter(
        (item) => item.readyforapproval === true,
      );
    }

    if (selectedApprovalState === "U") {
      filteredRows = filteredRows.filter(
        (character) => character.firstapprovalbyuserGuid === null,
      );
    }
    ...
    if (tagSelectValues.length > 0 || props.larpTags.length > 0) {
      ...
      filteredRows.forEach((row) => {
        if (row.tags !== null) {
          ...
          if (
            listguids.length === 0 ||
            props.larpTags.every((elem) => !listguids.includes(elem.guid)) ||
            listguids.includes(selectedLarpTag)
          ) {
            if (
              tagSelectValues.every((elem) => listguids.includes(elem.guid))
            ) {
              newfilter.push(row);
            }
          }
    ...
    if (props.commentFilterOn === true) {
      filteredRows = filteredRows.filter(
        (character) => character.hasreview === true,
      );
    }

    const filteredrowstotal = filteredRows.length;
    const filteredPageRows = filteredRows.filter(
      (series, index) =>
    ...
    for (const key of Object.keys(props.Filters)) {
      if (
        (key === "TagSelectValues" && props.Filters[key].length > 0) ||
    ...
    props.appdata,
    props.showallLARPLinked,
    props.showApprovableOnly,
    props.readyApproved,
    props.commentFilterOn,
    props.Filters.ItemsFilter,
    props.Filters.SeriesFilter,
    props.Filters.CreatorFilter,
    props.Filters.EditorFilter,
    tagState,
    selectedLarpTag,
  ...
  const tagClicked = (e) => {
    ...
    if (
      listguids.findIndex((element) => element === e) === -1 &&
    ...
    if (
      newtag !== undefined &&
      (tagSelectValues.length === 0 ||
        tagSelectValues.findIndex((element) => element.guid === e) === -1)
    ) {
    ...
    <div className="itemTable-display">
        ...
        >
          ...
          >
                ...
                <div className="table-controls">
                    ...
                    {!props.isSelector ? (
                      <>
                        <button
                          ...
                          onClick={(e) => props.NewItemLink(e)}
                        >
                          Add New Item
                        </button>
                        <button
                          className="button-action"
                          onClick={(e) => props.NavToSelectItems(e)}
                        >
                          Select/Print Items
                        </button>
                      </>
                  ...
                  {props.authLevel > 1 ? (
                    props.commentFilterOn !== undefined && !props.isSelector ? (
                      <FormControlLabel
                        control={
                          <Switch
                            defaultChecked={props.commentFilterOn}
                            onChange={() =>
                              props.ToggleSwitches({
                                commentFilter: !props.commentFilterOn,
                              })
                            }
                  ...
                  !props.isSelector ? (
                    <FormControlLabel
                      ...
                      label={
                        props.readyApproved ? "Ready for Approval" : "All Items"
                      }
                    />
                  ) : (
                    <></>
                  ...
                  !props.isSelector ? (
                    <FormControlLabel
                      control={
                        <Switch
                          defaultChecked={props.showApprovableOnly}
                          onChange={() =>
                            props.ToggleSwitches({
                      ...
                      <div className="selector">
                        <div className="input-pair">
                          <FormLabel sx={{ fontSize: 12 }}>
                            Include Items linked to Specific LARP
                          </FormLabel>
                          <Autocomplete
                            ...
                            renderInput={(params) => (
                              <TextField
                                ...
                                variant="outlined"
                                onChange={(e) =>
                                  UpdateLarpAutoComp(e.target.value)
                                }
                                InputProps={{
                                  ...params.InputProps,
                                  endAdornment: (
                                    <IconButton
                                      sx={{
                                        visibility:
                                          larpAutoCompValue !== "" &&
                                          larpAutoCompValue !== null
                                            ? "visible"
                                            : "hidden",
                                      }}
                                      onClick={() => UpdateLarpAutoComp("")}
                                    >
                                      <Clear />
                                    </IconButton>
                                  ),
                  ...
                  <div className="search-row">
                        ...
                        <div className="filter-container">
                          <Tooltip title="Filter by Last Editor">
                            <button
                              ...
                              onClick={() => FilterOpen("Editor")}
                            >
                              <FilterAltSharpIcon sx={{ fontSize: 23 }} />
                            </button>
                          </Tooltip>
                          <TextBox
                            title={"Sheet Editor Name"}
                            onClose={(e) => updateFilter(e, "Editor")}
                            value={
                      ...
                      <div className="search-container">
                        <div className="filter-container">
                          <Tooltip title="Filter by Sheet Creator">
                            <button
                              className="button-action-square"
                              onClick={() => FilterOpen("Creator")}
                            >
                              <FilterAltSharpIcon sx={{ fontSize: 23 }} />
                            </button>
                          </Tooltip>
                          <TextBox
                            title={"Sheet Creator Name"}
                            onClose={(e) => updateFilter(e, "Creator")}
                            value={
                          ...
                          props.Filters.CreatorFilter !== "" ? (
                            ...
                            <div className="inactive-text">Created By</div>
                          )}
                        </div>
                        <hr />
                      </div>
                    ...
                    <div className="search-container">
                      <div className="filter-container">
                        ...
                        props.Filters.ItemsFilter !== "" ? (
                          ...
                          <div className="inactive-text">Item Name</div>
                        )}
                      </div>
                      <hr />
                    </div>
                    <div className="search-container">
                      <div className="filter-container">
                        ...
                        <TextBox
                          title={"Series"}
                          onClose={(e) => updateFilter(e, "Series")}
                          value={
                        ...
                        props.Filters.SeriesFilter !== "" ? (
                          ...
                          <div className="inactive-text">Series</div>
                        )}
                      </div>
                      <hr />
                    </div>
            ...
            <TableRow>
              ...
              <TableCell
                key={Math.random()}
                className="icon-column table-cell-center"
              >
                {" "}
                View{" "}
              </TableCell>
                ...
                <TableRow key={Math.random()}>
                  {props.isSelector ? (
                    ...
                    <TableCell className="item-default-cursor table-cell-center">
                      {item.createdbyuserGuid !== null &&
                      item.firstapprovalbyuserGuid !== null &&
                      item.secondapprovalbyuserGuid !== null ? (
                      ...
                      item.secondapprovalbyuserGuid == null ? (
                        <div className="" title="unapproved: needs 2 approvals">
                          <HoverText
                            plainText={"U"}
                            hoverText={"Edit By: " + item.editbyUser}
                          />
                        </div>
                      ) : null}
                      {item.createdbyuserGuid !== null &&
                      ...
                      item.secondapprovalbyuserGuid == null ? (
                        <div
                          className=""
                          title="has 1st approval, needs 2nd approval"
                        >
                          <HoverText
                            plainText={"1st"}
                            hoverText={
                  ...
                  {props.isSelector ? (
                    ...
                    <TableCell className="table-default-cursor table-cell-center">
                      {item.readyforapproval !== null &&
                      item.readyforapproval === true
                        ? "Yes"
                        : "No"}
                    </TableCell>
                  )}
      ...
      {fullDeleteDialogOpen.open ? (
        <DeleteSanityDialog
          ...
          apiPath={{
            path: "ItemSheets",
            refreshName: "listUnapprovedItems",
            mutationName: "itemDelete",
          }}
