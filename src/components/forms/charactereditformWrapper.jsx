...
const CharacterEditFormWrapper = (props) => {
  ...
  const unsubscribeMutation = usePutData(
    "/api/v1/ReviewSub/StopItemSub/" + props.isSubbed.id,
  );

  const imgUrl = usePresignedImgQuery(
    "images/Characters/" + props.initForm.apiMessage.guid + ".jpg",
    props.path === "UnApproved" ||
      props.path === "CharacterSheets" ||
      props.initForm.apiMessage === undefined ||
      props.initForm.apiMessage === null ||
      props.initForm.apiMessage.img1 === undefined
      ? null
      : props.initForm.apiMessage.img1,
  );
  const imgUrl2 = usePresignedImgQuery(
    "images/Characters/" + props.initForm.apiMessage.guid + "_2.jpg",
    props.path === "UnApproved" ||
      props.path === "CharacterSheets" ||
      props.initForm.apiMessage === undefined ||
      props.initForm.apiMessage === null ||
      props.initForm.apiMessage.img2 === undefined
      ? null
      : props.initForm.apiMessage.img2,
  );

  if (imgUrl.isLoading || imgUrl2.isLoading)
    return (
      <div>
        <Loading />
      </div>
    );
  if (imgUrl.error !== null || imgUrl2.error !== null) return <div>Error!</div>;
