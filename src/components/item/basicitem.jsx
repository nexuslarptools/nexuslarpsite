...
const BasicItem = (props) => {
  const specialskillsallowed = ["Mecha", "Vehicle", "Companion", "Pokemon"];
            ...
            <div className="itemTextBox">
              <div className="seriesItemText">
                <div className="item-stats">
                  ...
                  {props.item.fields.INTEGRITY != null &&
                  props.item.fields.INTEGRITY !== "" ? (
                    <>
                      {" "}
                ...
                <div className="item-description">
                  {props.item.fields.Description !== undefined &&
                  props.item.fields.Description !== null ? (
                    props.item.fields.Description.split("\n").map((i, key) => {
                      return (
                        <div className="item-Description" key={key}>
                          <p>{formatText(i)}</p>
                        </div>
                      );
                    })
                ...
                specialskillsallowed.some(
                  (val) => val === props.item.fields.TYPE,
                ) ? (
                  <div>
                ...
                {props.item.fields.Magic != null &&
                props.item.fields.Magic != "" &&
                !props.item.isdoubleside ? (
                  <div>Magic: {props.item.fields.Magic}</div>
                ) : (
                  <></>
                )}
                {props.item.fields.Uses != null &&
                props.item.fields.Uses > 0 ? (
                  <div className="item-uses-container">
                    <div className="item-uses-text">
                      {" "}
                      Uses:
                      {Array.apply(null, {
                        length: props.item.fields.Uses,
                      }).map((e, i) => (
                        <span className="item-uses" key={i}>
                          ○
                        </span>
                      ))}
                    </div>
