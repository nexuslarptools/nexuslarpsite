...
const DisplayScreen = (props) => {
  ...
  const printFn = useReactToPrint({
    ...
    documentTitle:
      props.id === "character"
        ? props.character.name + " / " + props.character.seriesTitle
        : "items_" + Date().toLocaleString(),
  });

  return (
    <>
      <div className="sheet-option-buttons">
        <button className="button-action print-sheet-button" onClick={printFn}>
          Print This Sheet
        </button>
        {fontSize.fontSize === "normal" ? (
          <button
            className="change-font-size-large-button button-action"
          ...
          <button
            className="change-font-size-normal-button button-action"
            onClick={changeFontSize}
          >
            Normal Font Size
          </button>
        ...
        {fontType.fontType === "normal" ? (
          <button
            className="change-font-dyslexia-button button-action"
          ...
          <button
            className="change-font-default-button button-action"
            onClick={changeFontType}
          >
            Default Font
          </button>
        ...
        {isDoublesidePrint === true ? (
          <button
            className="change-print-single-side-print button-action"
            onClick={changePrintSides}
          >
            Single Side Print
          </button>
        ) : (
          <button
            className="change-print-double-side-print button-action"
            onClick={changePrintSides}
          >
            Double Side Print
          </button>
        ...
        <div className="sheet-print-active-states">
          <div className="sheet-print-active-states-label">
            Active Sheet States
          </div>
          <div>
            {fontSize.fontSize === "normal" ? (
          ...
          <div>
            {isDoublesidePrint === true ? (
              <span>
                <b>Print Style:</b> Double
              </span>
            ) : (
              <span>
                <b>Print Style:</b> Single
              </span>
            )}
          </div>
        </div>
        <div className="sheet-print-info">
          Note: On print, certain things may appear out of place if viewing it
          in the new browser window. They should move into correct place inside
          of the print view.
        </div>
      </div>
      <div ref={charComponent} className="character-sheet-printable">
        ...
        <div>
          <ItemPaletteWrapper
            printfrontback={isDoublesidePrint}
            itemsList={
              props.id === "character"
                ? props.character.starting_Items
                : props.itemList
            }
            itemsList2={
              props.id === "character" ? props.character.upgrade_Items : []
            }
            fullItems={props.id === "character" ? fullItems : props.itemList}
          />
        </div>
