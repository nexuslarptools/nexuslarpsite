...
# Try to get auth key from environment variable or secret file
...
if [ -n "$AUTH_KEY" ]; then
    ...
    tailscale up --authkey="$AUTH_KEY" --hostname="nexus-frontend" --ssh --accept-dns=false
else
    echo "Warning: TAILSCALE_AUTHKEY or TAILSCALE_OAUTH credentials not found. Checking if already authenticated..."
    if tailscale status >/dev/null 2>&1; then
        echo "Tailscale is already authenticated. Ensuring SSH is enabled..."
